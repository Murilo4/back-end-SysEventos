...
def get_event_and_questions(request, eventId):
    ...
    try:
        ...
        names = [Names.objects.get(id=username.name_id).name for username in username_list if Names.objects.filter(
    ...
    if not question_event_db:
        return JsonResponse({"success": False,
                             "message": "Nenhuma pergunta encontrada para este evento."},
                            status=status.HTTP_404_NOT_FOUND)
