...
# Carregar a chave secreta do JWT
load_dotenv()
...
def get_user_events(request):
    ...
    try:
        ...
        return JsonResponse({"success": False, "message": "Usuário não encontrado."}, status=status.HTTP_404_NOT_FOUND)

    events = Event.objects.filter(event_creator=user_id)

    # Verificação se há eventos
    if not events:
        return JsonResponse({"success": False, "message": "Nenhum evento encontrado."}, status=status.HTTP_404_NOT_FOUND)

    event_list = []
    for event in events:
        username_list = UserName.objects.filter(event_id=event.id).order_by('create_order')
    ...
    return JsonResponse({"success": True, "message": "Eventos encontrados com sucesso", "events": event_list},
